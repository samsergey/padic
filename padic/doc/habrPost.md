...
Для Haskell Андреем Лелеченко написан отличный пакет модулярной арифметики [mod](https://hackage.haskell.org/package/mod), с очень удобным интерфейсом и эффективной реализацией. Он и послужил основой для моей библиотеки. "Дорогое" разложение в виде списка цифр используется только для текстового представления чисел, вся арифметика производится в целочисленной модулярной арифметике.
...
Давайте найдём 10-адические автоморфные числа, то есть числа, квадрат которых (а значит и другие степени), равны им самим:
````hs
> x = henselLifting (\x -> x^2 - x) (\x -> 2*x - 1) :: [Z 10]
> x
