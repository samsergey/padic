...
Для перевода числа $inline$x = \frac{r}{s} \in Q$inline$ в $inline$\mathbb{Z}/p^k\mathbb{Z}$inline$ достаточно найти обратный элемент для $inline$s$inline$ в $inline$\mathbb{Z}/p^k\mathbb{Z}$inline$, то есть элемент $inline$q = s^{-1}$inline$, такой, что $inline$q\cdot s \equiv 1 \mod p^k$inline$. Это сравнение эквивалентно уравнению Безу $inline$s\cdot q + m\cdot p^k = 1$inline$, которое решается [расширенным алгоритмом Евклида](), еcли $inline$\gcd(s, p) = 1$inline$. 
...
1. Выделяем из числителя или знаменателя множитель $inline$p$inline$, то есть, представляем входное число $inline$x = \frac{r}{s}\cdot p^v$inline$.
  1.1. Если $inline$\gcd(s, p) = c \neq 1$inline$, то $inline$x = \frac{r'}{s'} \cdot p^{v+v'}$inline$, где $inline$s = s' \cdot c^{v'}$inline$,  $inline$r' = r \cdot (p / c)^{v'}$inline$.
2. Находим $inline$q = s^{-1} \mod p^k$inline$, решив уравнение Безу $inline$s\cdot q + m \cdot p^k = 1$inline$ расширенным алгоритмом Евклида или библиотечной функцией, наподобие `mpz_invert` из библиотеки GMP.
3. Результатом будет p-адическое число с обратимым элементом $inline$u = r\cdot q$inline$ и порядком $inline$v$inline$ .

**Пример 1.** Найдём образ числа $inline$x = 637/880$inline$ в $inline$\mathbb{Z}_7$inline$, округлённом так, чтобы можно было без потери точности восстанавливать числа с числителем и знаменателем, имеющими тип `Word16`.
...
Выделяем из числа степени основания: $inline$x = 7^2\cdot 13/880$inline$, значит, число будет иметь порядок $inline$2$inline$. Поскольку основание простое шаг 1.1 не потребуется.

Находим обратный элемент для знаменателя $inline$880^{-1} = 1242570101 \mod 7^{12}$inline$. Получаем обратимый элемент p-адического образа:

$$display$$u = 13 \cdot 1242570101 = 2312124112 \mod  7^{12}.$$display$$

**Пример 2.** Найдём образ числа $inline$x = 637/880$inline$ в $inline$\mathbb{Z}_{10}$inline$, с той же степенью округления.
...
Выделяем из числа степени основания: $inline$x = 10^{-1}\cdot 637/88$inline$. Основание не простое и имеет со знаменателем общий делитель $inline$2$inline$. Значит, дробь нужно преобразовать, избавившись от него. $$display$$\frac{637}{88} = \frac{1}{2^3}\frac{637}{11} = \frac{5^3}{10^3}\frac{637}{11} = 10^{-3}\frac{79625}{11}.$$display$$  Таким образом, получаем, что порядок числа равен $inline$ -1 -3 = -4$inline$.

Находим обратный элемент для знаменателя $inline$11^{-1} = 9090909091 \mod 10^{10}$inline$. Получаем обратимый элемент p-адического образа $$display$$u = 79625 \cdot 9090909091 = 3636370875 \mod 10^{10}.$$display$$ Кстати, для этого числа мы сразу можем записать его каноническое разложение: $$display$$x = ...363637.0875_{10}$$display$$

### Восстановление рационального числа из p-адического

Как мы уже говорили, восстановление рационального числа возможно не всегда.
